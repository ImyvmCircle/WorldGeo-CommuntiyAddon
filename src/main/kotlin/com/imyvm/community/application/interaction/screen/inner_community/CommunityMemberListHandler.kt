...
fun runCommunityOpenMember(community: Community, playerObjectUuid: UUID, playerExecutor: ServerPlayerEntity){
    val playerObjectProfile = UtilApi.getPlayerProfile(playerExecutor, playerObjectUuid)
    if (playerObjectProfile != null) {
        CommunityMenuOpener.open(playerExecutor) { syncId -> CommunityMemberMenu(syncId, community, playerObjectProfile, playerExecutor) }
    } else {
