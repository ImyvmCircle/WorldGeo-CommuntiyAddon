...
fun runSwitchMode(player: ServerPlayerEntity, cont: Any?){
    when (cont as? String) {
        ...
        "JOIN-ABLE" -> CommunityMenuOpener.open(player, "JOIN-ABLE") { syncId, content -> CommunityListMenu(syncId, content) }
        ...
        "REVOKED" -> CommunityMenuOpener.open(player, "REVOKED") { syncId, content -> CommunityListMenu(syncId, content) }
